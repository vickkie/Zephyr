...
export default function AnnouncementBanner() {
  ...
  useEffect(() => {
    const fetchAnnouncement = async () => {
      try {
        const res = await axios.get(`${VITE_SERVER}/api/announcements`, {
    ...
    if (!localStorage.getItem("dismissedBanner")) {
      fetchAnnouncement();
    }
  }, []);
  ...
  return (
    <div className=" position-relative back-primary">
      <div className="font-color p-1 text-center relative row back-primary">
        <div className="col-11">
          <p className="marquee">{announcement?.message}</p>
        </div>
        <div className=" d-flex col-1 justify-content-end cursor-pointer align-content-center">
          <div onClick={handleDismiss} className="right-2 top-2">
